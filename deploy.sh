...
find . ! -name keys.enc ! -name dist ! -name .git ! -name . -maxdepth 1 -exec rm -rf {} +
...
openssl aes-256-cbc -K $ENCRYPTED_KEY -iv $ENCRYPTED_IV -in keys.enc -out deploy_key -d
chmod 600 deploy_key
eval `ssh-agent -s`
ssh-add deploy_key

# Now that we're all set up, we can push.
git push -f -u $SSH_REPO $TARGET_BRANCH
